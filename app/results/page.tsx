...
// Retain your exact original 20-candidate database, even keeping the dirty data id_name perfectly
const CANDIDATES_DATABASE = [
  ...
  { id: "C015", name: "Rachel Lim", current_role: "AI Developer", desired_position: "AI Engineer", skills: ["Python", "OpenCV", "TensorFlow", "FastAPI"], years_experience: 4, region: "Malaysia", work_mode: "Remote" },
...
// Retain historical engagement feedback dataset
...
export default function ResultsPage() {
  ...
  // Registry state for persistently saving "reusable relationship entities"
  ...
  const calculateMatches = (req: any) => {
    ...
    const scoredCandidates = CANDIDATES_DATABASE.map((candidate) => {
      ...
      // Historical data evolution bonus points logic
      let networkBonus = 0;
      ...
      // 4 classic analytical descriptions
      const explanation = [
      ...
      // Encapsulate relationship entities
      const linkageEntity = {
  ...
  // 🌟 New core function: Decommission/disconnect ecosystem linkages to achieve complete programmable relationship lifecycle governance
  const handleRemoveLinkage = (linkageId: string) => {
    const updated = deployedLinkages.filter(l => l.linkageId !== linkageId);
    setDeployedLinkages(updated);
    localStorage.setItem("nexus_programmable_linkages", JSON.stringify(updated));
  };
    ...
    <main className="min-h-screen bg-black text-white p-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10">
          ...
          {/* Top ecosystem entity persistence dashboard */}
            ...
            {deployedLinkages.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {deployedLinkages.map((lnk) => (
                  // 🌟 Modification point: Add right padding to the tag and insert a lightweight X close button
                  <span key={lnk.linkageId} className="text-xs bg-blue-500/10 border border-blue-500/30 rounded-lg pl-3 pr-2 py-1.5 font-mono text-gray-300 flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse"></span>
                    <span>{lnk.linkageId} ({lnk.sourceNode} ⇄ {lnk.targetNode}) [{lnk.lifecycleState}]</span>
                    {/* 🌟 Core delete button: Click to safely remove from the governance framework, and link to restore the card button below */}
                    <button 
                      onClick={() => handleRemoveLinkage(lnk.linkageId)}
                      className="text-gray-500 hover:text-red-400 transition ml-1 p-0.5 rounded hover:bg-white/5"
                      title="Terminate Linkage Entity"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </span>
          ...
          {topMatches.map((candidate, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Programmable Relationship Entity Deployment Controller */}
